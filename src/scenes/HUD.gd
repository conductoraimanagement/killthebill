...
# Row formatters moved to PanelRows (class_name). HUD still owns the
# Panel + RichTextLabel + refresh scheduling; PanelRows just builds
# the BBCode string for each entry.
func _cameo_arc_row(arc: Dictionary) -> String:
	return PanelRows.cameo_arc_row(arc)


func _job_row(job: Dictionary) -> String:
	return PanelRows.job_row(job, WorldDirector.cycle)
...
func _wc_row(idx: int, listing: Dictionary) -> String:
	return PanelRows.wc_listing_row(idx, listing)


func _gig_row(slot: int, g: Dictionary) -> String:
	return PanelRows.gig_row(slot, g)
